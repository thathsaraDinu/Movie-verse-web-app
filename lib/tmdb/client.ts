...
export class TMDBClient {
  ...
  private async fetchWithRetry<T>(
    ...
    for (let attempt = 0; attempt <= retryAttempts; attempt++) {
      try {
        ...
        if (error instanceof TMDBRateLimitError || error instanceof TMDBNotFoundError) {
          throw error;
        }
    ...
    throw lastError || new TMDBError('Failed to fetch data from TMDB');
  }
  ...
  getDateRange(daysFromNow: number, daysToAdd: number = 0): { start: string; end: string } {
    const today = new Date();
    const start = new Date(today);
    start.setDate(today.getDate() + daysFromNow);
    const end = new Date(today);
    end.setDate(today.getDate() + daysFromNow + daysToAdd);
  ...
  // Helper method to get date from years ago
...
export function getTMDBClient(): TMDBClient {
  if (!clientInstance) {
    ...
    if (!apiKey) {
      throw new TMDBConfigError('TMDB_API_KEY environment variable is not set');
    }
